...
int main(int argc, char** argv) {
	...
	}; struct ficha_do_funcionario* funcionarioPtr, funcionario;
	...
	fflush(stdin);
	scanf_s("%[^\n]%*c", &funcionarioPtr->nome, 30);

	printf("Digite o salario do funcionario: (EX: 1500,99): ");
	fflush(stdin);
	scanf_s("%f", &funcionarioPtr->salario);

	printf("\n------ CALCULO DE ALIQUOTA DE IMPOSTO DE RENDA ------\n\n\n");

	printf("\nNome funcionario: %s \n", &funcionarioPtr->nome);
	calcula_imposto(&funcionarioPtr->salario);
